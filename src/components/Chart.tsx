...
const Chart: React.FC<ChartProps> = ({
  ...
  data.forEach((value, index) => {
    ...
    // Draw the value near the point
    ctx.fillStyle = textColor;
    ctx.font = '10px sans-serif';
    ctx.textAlign = 'center';
    ctx.textBaseline = 'bottom';
    ctx.fillText(value.toString(), x, y - 8); // Position the value slightly above the point
  });
    } else if (type === "bar") {
      // Draw bar chart
      ctx.clearRect(0, 0, canvas.width, canvas.height); // Clear canvas
      ...
      ctx.strokeStyle = gridColor; // Use gridColor for the axis
      ctx.lineWidth = 1;
      ctx.stroke();
  ...
  data.forEach((value, index) => {
    ...
    const x =
      canvas.width / 2 +
      Math.cos(midAngle) * (Math.min(canvas.width, canvas.height) / 2 - 40);
    const y =
      canvas.height / 2 +
      Math.sin(midAngle) * (Math.min(canvas.width, canvas.height) / 2 - 40);
    ...
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';
    ctx.fillText(`${labels[index]} (${percentage}%)`, x, y); // Append percentage to label
